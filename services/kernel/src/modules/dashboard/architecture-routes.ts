/**
 * Architecture map (topology + metrics). The dashboard reaches both through
 * `rpcOrCall` (`architecture.get` / `architecture.metrics` over the WS RPC,
 * `/api/architecture[/metrics]` otherwise), so each is one operation: the
 * RPC used to answer a different, much thinner shape (module names and
 * counts, no nodes/links; no heatmap), which the 3D page could not render.
 */

import type { KernelHttpServer, RouteMethod } from "../../core/http-server.js";
...
export interface ArchitectureOperationDeps {
  registry: ModuleRegistry;
  events: EventBus;
  getGraph: () => GraphDriver | null;
  sqlite: SqliteDb;
  systemRegistry: SystemRegistry;
  /** Live object: bootstrap fills it in once the RPC handler is up. */
  mtwRequest?: MtwRequestArchInfo | null;
}

export function registerArchitectureRoutes(httpServer: KernelHttpServer, opts: ArchitectureOperationDeps): void {
  const op = architectureOperations(opts);
  const bind = ([method, path, name]: [RouteMethod, string, string]) => httpServer.operation(method, path, op[name]);
  ([
    ["GET", "/api/architecture", "architecture.get"],
    ["GET", "/api/architecture/metrics", "architecture.metrics"],
  ] as Array<[RouteMethod, string, string]>).forEach(bind);
}

export function architectureOperations(opts: ArchitectureOperationDeps): Record<string, Operation> {
  const { registry, events, getGraph, sqlite, systemRegistry } = opts;
  const mtw = opts.mtwRequest;

  return {
    // Architecture 3D map — full system topology
    "architecture.get": () => {
    ...
    // Architecture metrics — heatmap, health, timeline
