...
export function registerMarketplaceRoutes(
  ...
  server.route("GET", "/api/agents/marketplaces", async () => {
    const { readdir, readFile } = await import("node:fs/promises");
    ...
    if (!existsSync(marketplacesDir)) return { marketplaces: [], root: marketplacesDir };
    ...
    return { marketplaces: items, root: marketplacesDir };
  });

  // POST /api/agents/marketplaces — clone a marketplace from a git URL
  server.route<{ url: string; name?: string; confirm?: boolean; acknowledged?: boolean }>("POST", "/api/agents/marketplaces", async ({ body }) => {
    if (!body.url || typeof body.url !== "string") throw new HttpError(400, "url required");
    ...
    if (!validProtocol) {
      throw new HttpError(400, "Only https URLs from github.com, gitlab.com, codeberg.org, or bitbucket.org are accepted");
    }
    ...
    if (!confirmed) {
      throw new HttpError(428, "confirmation_required", {
    ...
    if (!/^[A-Za-z0-9_.-]{1,80}$/.test(dirName)) throw new HttpError(400, "invalid marketplace name");

    const targetPath = resolvePath(marketplacesDir, dirName);
    if (existsSync(targetPath)) throw new HttpError(409, `A marketplace with that name already exists: ${dirName}`);
    ...
    try {
      ...
      throw new HttpError(500, `git clone failed: ${msg.slice(0, 300)}`);
    }
    ...
    if (!hasPlugins && !hasManifest) {
      ...
      throw new HttpError(400, "The repo doesn't look like a Claude Code marketplace (missing plugins/ or .claude-plugin/marketplace.json)");
    }
  ...
  /** The path of the cloned marketplace `name`: 400 for a name that is not a plain slug, 404 when it is not there. */
  async function existingMarketplace(name: string): Promise<string> {
    if (!/^[A-Za-z0-9_.-]{1,80}$/.test(name)) throw new HttpError(400, "invalid name");
    const { existsSync } = await import("node:fs");
    const { resolve: resolvePath } = await import("node:path");
    const hostHome = process.env.HOST_HOME ?? homedir();
    const mpPath = resolvePath(hostHome, ".claude/plugins/marketplaces", name);
    if (!existsSync(mpPath)) throw new HttpError(404, "marketplace not found");
    return mpPath;
  }

  // POST /api/agents/marketplaces/:name/refresh — git pull
  server.route("POST", "/api/agents/marketplaces/:name/refresh", async ({ params: { name } }) => {
    const mpPath = await existingMarketplace(name);
    const { execFileSync } = await import("node:child_process");
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      throw new HttpError(500, `git pull failed: ${msg.slice(0, 300)}`);
    }
  });

  // DELETE /api/agents/marketplaces/:name — remove a marketplace
  server.route("DELETE", "/api/agents/marketplaces/:name", async ({ params: { name } }) => {
    const mpPath = await existingMarketplace(name);
    ...
    try {
      ...
      throw new HttpError(500, `rm failed: ${String(err)}`);
    }
  });
  ...
  server.route("GET", "/api/agents/extensions", async () => {
