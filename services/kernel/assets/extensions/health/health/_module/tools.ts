...
export function healthTools(service: HealthService): ToolDefinition[] {
  return [
    defineTool({
      ...
      handler: async (input) => {
        const metric = service.logMetric(input);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const med = service.addMedication(input);
    ...
    defineTool({
      ...
      handler: async ({ id, active, ...rest }) => {
        const changes = active !== undefined ? { ...rest, active: active ? 1 : 0 } : rest;
    ...
    defineTool({
      ...
      handler: async (input) => {
        const apt = service.addAppointment(input);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const apts = service.listAppointments(input);
