import { type SqliteDb, today, daysFromNow, toRecord } from "@kernl/extension-sdk";
...
export function queryFinance(db: SqliteDb): DashboardFinance | null {
  try {
    db.prepare("SELECT 1 FROM finance_accounts LIMIT 0").get();
  } catch {
    return null;
  }
  ...
  try {
    const budgets = db
      .prepare(`SELECT id, name, category, amount_cents, period FROM finance_budgets`)
      .all() as Array<{ id: string; name: string; category: string; amount_cents: number; period: string }>;
    ...
    for (const b of budgets) {
      ...
      if (b.period === "weekly") {
        ...
        const m = new Date(todayStr + "T12:00:00Z").getUTCMonth();
        const qStart = m - (m % 3);
        periodStart = `${todayStr.slice(0, 4)}-${String(qStart + 1).padStart(2, "0")}-01`;
      } else if (b.period === "yearly") {
        periodStart = `${todayStr.slice(0, 4)}-01-01`;
      } else {
        periodStart = `${todayStr.slice(0, 7)}-01`; // monthly
      }
