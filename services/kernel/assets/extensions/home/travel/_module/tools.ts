import { z } from "zod";
import { defineTool, textResult, errorResult, type ToolDefinition, formatCents } from "@kernl/extension-sdk";
...
export function travelTools(svc: TravelService): ToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      handler: async (input) => {
        const trip = svc.createTrip(input);
        return textResult(
    ...
    defineTool({
      ...
      handler: async (filters) => {
        const trips = svc.listTrips(filters);
    ...
    defineTool({
      ...
      handler: async ({ id, ...changes }) => {
        const trip = svc.updateTrip(id, changes);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const flight = svc.addFlight(input);
        return textResult(
    ...
    defineTool({
      ...
      handler: async ({ id, ...changes }) => {
        const flight = svc.updateFlight(id, changes);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const a = svc.addAccommodation(input);
    ...
    defineTool({
      ...
      handler: async ({ id, ...changes }) => {
        const a = svc.updateActivity(id, changes);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const item = svc.addPackingItem(input);
    ...
    defineTool({
      ...
      handler: async (input) => {
        const doc = svc.addDocument(input);
